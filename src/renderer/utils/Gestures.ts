...
export class LandMarksMethods {
    ...
    constructor(results: HandLandmarkerResult, w: number, h: number) {
    ...
    static findPosition(results: NormalizedLandmarkList, w: number, h: number) {
        const landmarks: FingerPosition[] = [];
        if (results) {
            results.map((landmark) => {
                landmarks.push({
                    x: Math.round(landmark.x * w),
                    y: Math.round(landmark.y * h),
                });
            });
        }
        return landmarks;
